...
class RepositoryTest {
	...
	void userCreateReadUpdateDelete() {
		...
		var data = new User.Data(
				...
				"Alice Smith",
				Arrays.asList("notification 0", "notification 1"),
				Collections.singletonMap(new BookRequest("Clean Code", "Robert C. Martin"), new BookRequest.Data(new Date()))
		);

		// ---- create ---------------------------------------------------------
		assertDoesNotThrow(() -> service.createUser(user, data),
				"createUser should not throw when the key is new");

		// Trying to insert the same user again must fail
		assertThrows(Repository.TransactionException.class,
				() -> service.createUser(user, data),
				"createUser should throw for duplicate keys");
		...
		// ---- update ---------------------------------------------------------
		Function<User.Data, User.Data> deactivateAccount =
				d -> d.withActive(false);
		...
		// ---- delete ---------------------------------------------------------
		assertDoesNotThrow(() -> service.deleteUser(user),
				"deleteUser must not throw when the key existed");
		assertThrows(Repository.TransactionException.class,
				() -> service.deleteUser(user),
				"deleteUser must throw for a non‑existent key");
		assertThrows(Repository.TransactionException.class,
				() -> service.updateUser(user, deactivateAccount));
	}

	@Test
	void bookCreateReadUpdateDelete() {
		var book = new Book("Clean Code", new Author.ByName("Robert C. Martin"));
		var data = new Book.Data(
				"Good book!", Book.ApprovalStatus.APPROVED,
				null,
				Collections.singletonMap(new User("alice"), new Book.Borrow(
						new Date(),
						Duration.ofMillis(42),
						new ByteArray(new byte[42])
				)),
				5);
		var book2 = new Book("Dirty Code", new Author.ByRef(new User("alice")));
		var data2 = new Book.Data(
				"Bad book!", Book.ApprovalStatus.REJECTED,
				new Book("Clean Code", new Author.ByRef(new User("alice"))),
				Collections.singletonMap(new User("alice"), new Book.Borrow(
						new Date(),
						Duration.ofMillis(84),
						new ByteArray(new byte[84])
				)),
				0);

		// ---- create ---------------------------------------------------------
		assertDoesNotThrow(() -> service.createBook(book, data),
				"createBook should not throw when the key is new");
		// Trying to insert the same book again must fail
		assertThrows(Repository.TransactionException.class,
				() -> service.createBook(book, data),
				"createBook should throw for duplicate keys");
		assertDoesNotThrow(() -> service.createBook(book2, data2),
				"createBook should not throw when the key is new");
		...
		// ---- update ---------------------------------------------------------
		Function<Book.Data, Book.Data> addTimesBorrowed =
				d -> d.withTimesBorrowed(d.timesBorrowed() + 3);
		...
		assertEquals(8, afterUpdate.timesBorrowed(),
				"The callback should have increased times borrowed to 3");

		// ---- delete ---------------------------------------------------------
		assertDoesNotThrow(() -> service.deleteBook(book),
				"deleteBook must not throw when the key existed");
		assertThrows(Repository.TransactionException.class,
				() -> service.deleteBook(book),
				"deleteBook must throw for a non‑existent key");
		assertThrows(Repository.TransactionException.class,
				() -> service.updateBook(book, addTimesBorrowed));
	}
}
