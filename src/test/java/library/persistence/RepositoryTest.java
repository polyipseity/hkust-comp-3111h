...
class RepositoryTest {
	...
	void transact_missingUserConstraint() {
		...
		assertThrows(TransactionException.class, () -> repository.transact(tx -> {
			tx.borrows().put(new Object[]{missingUser, existingBook}, new Borrow(TimeUtil.nowZoned(), Duration.ofMinutes(42), "test.pdf"));
			return true;
		}));
	...
	void transact_missingBookConstraint() {
		...
		assertThrows(TransactionException.class, () -> repository.transact(tx -> {
			tx.borrows().put(new Object[]{existingUser, missingBook}, new Borrow(TimeUtil.nowZoned(), Duration.ofMinutes(42), "test.pdf"));
			return true;
		}));
	}
	...
	void transact_removeUserConstraint() {
		...
		final var borrowToRemove = new Borrow(TimeUtil.nowZoned(), Duration.ofMinutes(42), "test.pdf");
	...
	void transact_removeBookConstraint() {
		...
		final var newBorrowToRemove = new Borrow(TimeUtil.nowZoned(), Duration.ofMinutes(42), "test.pdf");
